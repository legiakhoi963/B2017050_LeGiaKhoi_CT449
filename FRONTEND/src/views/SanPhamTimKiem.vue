<template>
    <div>
     ...
     <main>
        ...
        <!-- phan cach -->
        <div class="container mt-3" id="phancach">

            <p class="tenloai">Kết quả tìm kiếm: {{this.tenhh}}</p>
  ...
  export default {
    ...
    watch: {
      $route: ["getSanPham"], // Theo dõi thay đổi đường dẫn và gọi getSanPham khi thay đổi
  },
   ...
   // Default to false if this.sanpham is null
      ...
      async getSanPham(){
        const tenhh = this.$route.query.tenhh;
        this.tenhh = this.$route.query.tenhh;
        console.log(tenhh);
        try{
           this.listsanpham = await hanghoaService.searchByName(tenhh);
